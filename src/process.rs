...
fn err_ctx<P: AsRef<Path>>(e: impl Into<ProcessErrorKind>, p: P) -> ProcessError {
    let msg = format!("IO error running entrypoint at path: {:?}", p.as_ref());
    ProcessError {
        msg,
        source: e.into(),
    }
...
pub(crate) fn run_entrypoint<P: AsRef<Path>>(
    ...
    let mut entrypoint_output = Cmd::new(&program_path)
        ...
        .stdout(Stdio::piped())
        .spawn()
        .map_err(|e| err_ctx(e, &program_path))?;
